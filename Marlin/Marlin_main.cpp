...
/* Rajat Changes */
#if ENABLED(LED_CONTROL)
int extruder_led_status = 0;
int bed_led_status = 0;
float old_hotend_temp = 0;
float present_hotend_temp = 0;
float old_bed_temp = 40;
float present_bed_temp = 0;
#endif
...
void process_next_command() {
  ...
  switch (parser.command_letter) {
	...
	#if ENABLED(AUDIO_OUTPUT)			// Buzzer Sound Routine
		  /* RAJAT CHNAGES */
		case 'B': switch (parser.codenum) {
		case 0:
			SERIAL_PROTOCOLLN("BUZZER ON");
...
#if ENABLED(LED_CONTROL)		
void led_control_procedure()
{
#if EXTRUDERS == 2
	int ext1_temp = 0;
	int ext2_temp = 0;
	ext1_temp = thermalManager.degHotend(0);
	ext2_temp = thermalManager.degHotend(1);
	if (ext1_temp >= ext2_temp)
	{
		present_hotend_temp = ext1_temp;
	}
	else
	{
		present_hotend_temp = ext2_temp;
	}
#else
	present_hotend_temp = thermalManager.degHotend(active_extruder);
#endif

	present_bed_temp = thermalManager.degBed();
	if (present_hotend_temp >= HOTEND_THRESHOLD_TEMP)
	{
		if ((present_hotend_temp - old_hotend_temp) > LED_TEMP_DIFF)
		{
			if (!extruder_led_status)
			{
				extruder_led_status = 1;
				#if ENABLED(HANDOVER_SUCCESS)
				WRITE(EXTRUDER_LED, HIGH);
				#endif	
				SERIAL_PROTOCOLLN("EXTRUDER LED ON");
				old_hotend_temp = present_hotend_temp;
			}
		}
	}
	else
	{
		if ((old_hotend_temp - present_hotend_temp) > 3)
		{
			if (extruder_led_status)
			{
				extruder_led_status = 0;
				#if ENABLED(HANDOVER_SUCCESS)
				WRITE(EXTRUDER_LED, LOW);
				#endif
				SERIAL_PROTOCOLLN("EXTRUDER LED OFF");
				old_hotend_temp = present_hotend_temp;
			}
		}
	}

	if (present_bed_temp >= BED_THRESHOLD_TEMP)
	{
		if ((present_bed_temp - old_bed_temp) > 3)
		{
			if (!bed_led_status)
			{
				bed_led_status = 1;
				...
				WRITE(BED_LED, HIGH);
				#endif
				SERIAL_PROTOCOLLN("BED LED ON");
				old_bed_temp = present_bed_temp;
			}
		}
	}
	else
	{
		if ((old_bed_temp - present_bed_temp) > 3)
		{
			if (bed_led_status)
			{
				bed_led_status = 0;
...
void setup() {
  ...
  lcd_init();
	...
	// Rajat Changes
	#if ENABLED(LED_CONTROL)		// Rajat Changes
		OUT_WRITE(EXTRUDER_LED, LOW);
		OUT_WRITE(BED_LED, LOW);
	#endif

	#if ENABLED(AUDIO_OUTPUT)		// Rajat Changes
...
void loop() {
  ...
  if (commands_in_queue) {
	...
	// Rajat changes
	#if ENABLED(LED_CONTROL)		//Bed Led control
	  led_control_procedure();		//Bed Led Control Routine
	#endif
